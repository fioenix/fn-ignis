...
def test_channel_summary_generation_covers_all_five_platforms():
    ...
    assert "120.0K lượt xem" in by_platform["youtube"].top_citation.metric_highlight
    assert by_platform["youtube"].top_citation.author_or_channel == "Vincent AI"
    assert by_platform["youtube"].timeframe_used == "30d (VN)"

    assert by_platform["google"].top_citation.metric_highlight.startswith("chỉ số tìm kiếm 95/100")
    ...
    for empty in ("threads", "reels"):
        ...
        assert "Không có tín hiệu khớp từ khóa" in by_platform[empty].notes
...
def test_voice_of_customer_insight_cites_the_discussed_assets():
    ...
    voc = [i for i in report.strategic_insights if "Tiếng nói khách hàng" in i.statement]
    assert voc, "Comment-bearing signals must produce a Voice of Customer insight."
    assert "80 phản hồi thu được" in voc[0].statement
...
def test_partial_ingress_coverage_is_reported_as_an_insight():
    ...
    coverage = [i for i in report.strategic_insights if "Độ phủ thu thập chưa đầy đủ" in i.statement]
    assert coverage
    assert "THREADS (AUTH_REQUIRED)" in coverage[0].statement
    assert any("Khôi phục các kênh thu thập đang rỗng" in a for a in report.actionable_takeaways)
...
def test_dynamic_report_copy_is_rendered_in_vietnamese():
    """
    Template labels stay English; every dynamic string injected into it must
    reach a Vietnamese-market reader in Vietnamese.
    """
    reasoner = StrategicMarketReasoner()
    evaluator = QualityEvaluator()
    signals = _signals()
    ...
    assert report.strategic_insights[0].statement.startswith("Độ trưởng thành thị trường")
    assert any("Lập lịch thu thập định kỳ" in a for a in report.actionable_takeaways)
    assert all(
        any(marker in o.strategic_recommendation for marker in ("Chỉ số Cơ hội", "cân bằng"))
        for o in report.market_opportunities
    )
    for ch in report.channel_summaries:
        if ch.notes:
            assert any(w in ch.notes for w in ("Không", "Chưa", "Kênh", "Circuit Breaker"))
        if ch.top_citation:
            assert any(
                unit in ch.top_citation.metric_highlight
                for unit in ("lượt xem", "chỉ số tìm kiếm", "lượt tương tác")
            )
