...
def test_crypto_ephemeral_key_security(monkeypatch):
...
def test_bootstrap_generated_configs_are_gitignored():
    """Verify that all MCP config paths generated by bootstrap are properly protected in .gitignore."""
    import subprocess
    from pathlib import Path

    project_root = Path(__file__).resolve().parents[2]
    critical_configs = [".mcp.json", ".cursor/mcp.json", ".vscode/mcp.json"]

    for config_rel in critical_configs:
        res = subprocess.run(
            ["git", "check-ignore", "-q", config_rel],
            cwd=str(project_root)
        )
        assert res.returncode == 0, f"Generated config '{config_rel}' is NOT protected by .gitignore!"
