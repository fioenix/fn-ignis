...
async def test_bug07_summary_matches_signal_count_across_timeframes(tmp_path):
    ...
    # 2 signals trong 24h
    # Each signal names the external object it observed; one that names nothing is not stored
    # as an observation and so cannot be counted in a window.
    s1 = TrendSignal(platform=PlatformType.GOOGLE_TRENDS, raw_title="AI 1", cluster_id=cluster_id, captured_at=now - timedelta(hours=2), metadata={"keyword": "ai 1"})
    s2 = TrendSignal(platform=PlatformType.YOUTUBE, raw_title="AI 2", cluster_id=cluster_id, captured_at=now - timedelta(hours=5), metadata={"video_id": "dQw4w9WgXcQ"})
    # Two signals inside 7d but outside 24h
    s3 = TrendSignal(platform=PlatformType.TIKTOK, raw_title="AI 3", cluster_id=cluster_id, captured_at=now - timedelta(days=3), metadata={"item_id": "7300000000000000001"})
    s4 = TrendSignal(platform=PlatformType.THREADS, raw_title="AI 4", cluster_id=cluster_id, captured_at=now - timedelta(days=5), metadata={"post_id": "P1"})
    ...
    match_24 = re.search(r"^(\d+)\b", c24.summary_text)
    ...
    match_7 = re.search(r"^(\d+)\b", c7.summary_text)
